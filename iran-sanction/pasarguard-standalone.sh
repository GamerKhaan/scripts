...
set_pasarguard_panel_image() {
    ...
    sed -i -E "0#^[[:space:]]*image:[[:space:]]*(pasarguard/panel|ghcr.io/gamerkhaan/panel):.*#s##    image: ${target_image}#" "$COMPOSE_FILE"
}
...
install_pasarguard() {
    ...
    target_image="ghcr.io/gamerkhaan/panel:${pasarguard_version}"
    if [ "$pasarguard_version" = "latest" ]; then
        target_image="ghcr.io/gamerkhaan/panel:latest"
    fi
